...
void RGBParamHandler::declareParams(rclcpp::Node* node, std::shared_ptr<dai::node::ColorCamera> color_cam) {
    declareAndLogParam(node, "i_max_q_size", 4);
    declareAndLogParam(node, "i_publish_preview", false);
    declareAndLogParam(node, "i_board_socket_id", 0);
    color_cam->setFps(declareAndLogParam<double>(node, "i_fps", 30.0));
    size_t preview_size = declareAndLogParam(node, "i_preview_size", 256);
    color_cam->setPreviewSize(preview_size, preview_size);
    color_cam->setVideoSize(declareAndLogParam(node, "i_rgb_width", 1920), declareAndLogParam(node, "i_rgb_height", 1080));
    ...
    size_t whitebalance = declareAndLogParam(node, "r_whitebalance", 1000, get_ranged_int_descriptor(3300, 6000));
