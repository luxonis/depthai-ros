...
void RGB::setOutputs(std::shared_ptr<dai::Pipeline> pipeline) {
    ...
    if(outputIsp && !lowBandwidth) {
        rgbPub = createPublisher(pipeline, ispQName, colorCamNode->isp, ph->getParam<bool>("i_synced"), encConfig);
    } else {
        rgbPub = createPublisher(pipeline, ispQName, colorCamNode->video, ph->getParam<bool>("i_synced"), encConfig);
    }

    }
    if(ph->getParam<bool>("i_enable_preview")) {
        previewPub = createPublisher(pipeline, previewQName, colorCamNode->preview);
    }
}
...
void RGB::setupQueues(std::shared_ptr<dai::Device> device) {
    if(ph->getParam<bool>("i_publish_topic")) {
        ...
        convConfig.encoding = dai::ImgFrame::Type::BGR888i;
	...
	controlQ = colorCamNode->inputControl.createInputQueue();
}
