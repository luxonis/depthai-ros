...
void Mono::setOutputs(std::shared_ptr<dai::Pipeline> pipeline) {
    if(ph->getParam<bool>("i_publish_topic")) {
        ...
        imagePublisher = createPublisher(
            pipeline, monoQName, monoCamNode->out, ph->getParam<bool>("i_synced"), encConfig);
    }
...
void Mono::setupQueues(std::shared_ptr<dai::Device> device) {
    if(ph->getParam<bool>("i_publish_topic")) {
        ...
        convConf.encoding = dai::ImgFrame::Type::GRAY8;
