...
from sensor_msgs.msg import CompressedImage, Image, Imu, CameraInfo  # , PointCloud2, PointField, Imu  # s, PointCloud
from std_msgs.msg import Header
from builtin_interfaces.msg import Time
from depthai_sdk.components.parser import parse_camera_socket
from depthai import CameraBoardSocket

from .imu_interpolation import ImuInterpolation
...
class DepthAi2Ros2:
    ...
    def get_calib(self, name: str, msg):
        cal_handler = self.device.readCalibration()
        if name == 'depth': name = 'left'
        socket = parse_camera_socket(name)
        info = CameraInfo()
        info.distortion_model = "rational_polynomial"
        intrinsics = cal_handler.getCameraIntrinsics(socket)
        info.k =  [item for sublist in intrinsics for item in sublist]
        info.d = cal_handler.getDistortionCoefficients(socket)
        extrinsics = cal_handler.getCameraExtrinsics(CameraBoardSocket.RIGHT, CameraBoardSocket.LEFT)
        info.p =  [item for sublist in intrinsics for item in sublist]
        # print("$$$$$$$")
        # print(extrinsics)
        # print("######")
        # print(intrinsics)
    ...
    def set_header(self, msg, dai_msg: Union[dai.ImgFrame, dai.IMUReport], frame: str) -> Header:
        msg.header.frame_id = frame  # ImgFrame
    ...
    def CompressedImage(self, imgFrame: dai.ImgFrame, frame: str) -> CompressedImage:
        msg = CompressedImage()
        self.set_header(msg, imgFrame, frame)
        msg.format = "jpeg"
        msg.data.frombytes(imgFrame.getData())
        return msg

    def Image(self, imgFrame: dai.ImgFrame, frame: str) -> Image:
        msg = Image()
        self.set_header(msg, imgFrame, frame)
    ...
    def Imu(self, dai_msg, frame: str):
        dai_msg: dai.IMUData
        for packet in dai_msg.packets:
            ...
            self.set_header(msg, report, frame)
